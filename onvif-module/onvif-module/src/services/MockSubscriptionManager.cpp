...
std::string MockSubscriptionManager::handleGetEventProperties(const std::string& req) {
    ...
    std::string body =
        ...
        // TopicSet: topic leaf với MessageDescription chứa Source+Data
        // SimpleItemDescription để test tool build filter expression được.
        // Type dùng "xsd:" (không phải "xs:") — tool chỉ resolve namespace này.
        "<wstop:TopicSet>"
          "<tns1:VideoSource>"
          ...
          "<tns1:Media>"
            ...
            "<tns1:ConfigurationChanged wstop:topic=\"true\">"
              "<tt:MessageDescription IsProperty=\"false\">"
                ...
                "</tt:Source>"
                "<tt:Data>"
                  "<tt:SimpleItemDescription Name=\"ConfigToken\" Type=\"tt:ReferenceToken\"/>"
                "</tt:Data>"
              "</tt:MessageDescription>"

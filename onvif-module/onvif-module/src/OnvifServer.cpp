...
static void proxyRtspHttpTunnel(int clientFd) {
    ...
    while (alive) {
        ...
        int pr = ::poll(fds, 2, 120000);   // 120s idle → đóng
        if (pr <= 0) break;
        // client → relay
        ...
        // Chỉ đóng khi lỗi/hangup thực sự (không đóng vì kênh kia im lặng).
        if ((fds[0].revents | fds[1].revents) & (POLLERR | POLLNVAL)) break;
    }
    ::close(relay);
    ::close(clientFd);
}
...
// NGAY — với request lớn (SetMetadataConfiguration có <Events>, Set config...) body
// có thể chưa tới đủ (TCP segmentation) → g_current_headers bị cắt → dispatch
// string-match trượt → fallthrough gSOAP → fault "Data required" (MEDIA2-8-1-1
// chập chờn: pass khi chạy riêng, fail trong full run). Dùng MSG_PEEK (KHÔNG tiêu
// thụ) nạp phần body còn thiếu; gSOAP vẫn đọc socket bình thường sau nếu
// fallthrough. Chỉ làm việc khi body thiếu — request đủ/nhỏ là no-op. An toàn với
// streaming vì Media2 Set không còn respawn (780c1d6) → không gây churn.
...
void OnvifServer::listenLoop() {
    ...
    while (running_) {
        ...
        if (beginResult == SOAP_OK) {
            ...
            // request (MEDIA2-8-1-1). Non-consuming (MSG_PEEK) → gSOAP fallthrough
